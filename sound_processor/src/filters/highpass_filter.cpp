...
HighpassFilter::HighpassFilter(int windowSize): _windowSize(windowSize)
{
    validateOddWindow(_windowSize, "highpass");
}
...
void HighpassFilter::apply(Waveform& waveform)
{
    ...
    for(size_t i = 0; i < samples.size(); ++i)
    {
        result[i] = clampSample(static_cast<double>(samples[i]) -
                                movingAverage(samples, i, HALF, _windowSize));
    }

    samples = std::move(result);
}

int HighpassFilter::getWindowSize() const { return _windowSize; }
