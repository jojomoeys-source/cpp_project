...
std::vector<int16_t> bandpassSignal(const std::vector<int16_t>& samples,
                                    int lowWindowSize, int highWindowSize)
{
    const std::vector<int16_t> LOWPASSED =
        movingAverageSignal(samples, lowWindowSize);
    const int HIGH_HALF = highWindowSize / 2;
    std::vector<int16_t> result(LOWPASSED.size());

    for(size_t i = 0; i < LOWPASSED.size(); ++i)
    {
        const double LOW =
            movingAverage(LOWPASSED, i, HIGH_HALF, highWindowSize);
...
RejectFilter::RejectFilter(int lowWindowSize, int highWindowSize)
    : _lowWindowSize(lowWindowSize), _highWindowSize(highWindowSize)
{
    validateOddWindow(_lowWindowSize, "reject low_window");
    validateOddWindow(_highWindowSize, "reject high_window");
}
...
void RejectFilter::apply(Waveform& waveform)
{
    ...
    const std::vector<int16_t> BAND =
        bandpassSignal(samples, _lowWindowSize, _highWindowSize);
    std::vector<int16_t> result(samples.size());

    for(size_t i = 0; i < samples.size(); ++i)
    {
        result[i] = clampSample(static_cast<double>(samples[i]) -
...
int RejectFilter::getLowWindowSize() const { return _lowWindowSize; }

int RejectFilter::getHighWindowSize() const { return _highWindowSize; }
